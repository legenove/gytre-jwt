...
function _M.create_preload(o)
    ...
    local _preload = util:table_copy_table(preload)
    util.table_merge(_preload,o)
    return _preload
end

function _M:encode(data, alg)
	if type(data) ~= 'table' then return nil, "Argument #1 must be table" end
    data = util.table_dict_sort(data)
    local key
	alg = alg or "HS256"
    if util.startswith(alg,'HS') then
        key = self.SECRET_KEY
    else
...
function _M:decode(data, verify)
	...
	if verify then
        ...
        if util.startswith(header.alg, 'HS') then
            key = self.SECRET_KEY
        else
